...
author: "Alicia Esteban Aicart"
date:  "14/02/2022"  #Pondría la fecha del día actual
params:
  lang: ES
...
```{r}
commonAnimals<- intersect(df_mammals, df_animals)
...
```{r}
noestan_mammals<- setdiff(rownames(df_mammals),rownames(df_animals))
noestan_mammals
```

```{r}
noestan_animals<- setdiff(rownames(df_animals),rownames(df_mammals))
noestan_animals
...
```
```{r}
#c repetimos proceso ahora con el conjunto de datos chile
head(Chile)
dim(Chile)
tail(Chile)
glimpse(Chile)
str(Chile)
#ahora los valores perdidos
any(is.na(Chile))
...
```{r}
ggplot(cabbages,aes(VitC, fill=Cult)) + geom_histogram(alpha= 2.5)
